...
from fusion_bench.method.tall_mask.utils import (construct_consensus_mask,
                                                 generate_task_masks)
from fusion_bench.method.ties_merging.ties_merging_utils import (
    state_dict_to_vector, ties_merging, vector_to_state_dict)
...
class ContinualConsensusTAForCLIP(BaseAlgorithm, LightningFabricMixin):
    def __init__(
        self,
    ...
    def run(self, modelpool: BaseModelPool):
        ...
        for model_idx, model_name in tqdm(
            ...
            if model_idx == 0:
                ...
                merged_state_dict = task_vector
                tall_mask_o = deepcopy(
                ...
                tall_mask_n = generate_task_masks(
                    tv_flat_checks,
                    state_dict_to_vector(task_model.state_dict()),
                    state_dict_to_vector(pretrained_model.state_dict()),
                    tall_mask_lambda=self.lmda,
                )

                tall_masks = [tall_mask_o, tall_mask_n.float()]

                consensus_mask = torch.sum(torch.stack(tall_masks), dim=0)
                # filter out the least-activated parameters based on given threshold
                consensus_mask = (consensus_mask >= self.k).float()

                merging_tv = consensus_mask * tv_flat_checks.sum(0)
            ...
            for param_name, param in task_model.named_parameters():
                ...
                new_param = (
                    merged_param + self.scaling_factor * merged_state_dict[param_name]
                )
