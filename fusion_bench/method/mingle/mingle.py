...
class MINGLE(BaseAlgorithm, LightningFabricMixin):
    def __init__(
        self,
    ...
    def run(self, modelpool: BaseModelPool):
        ...
        merged_model = deepcopy(pretrained_model.cpu()).to(accelerator)
        ...
        for model_idx, model_name in tqdm(enumerate(model_names)):
            ...
            merged_model = self.test_time_adaptation(
                merged_model, task_model, model_name, self._config.seed_sample_number
            )

            if self._config.save_gate_state and model_idx == len(model_names) - 1:
                for model_name in model_names[: model_idx + 1]:
                    self._compute_and_save_gate_stats(
                        merged_model, model_name, model_idx
                    )

            torch.cuda.empty_cache()

            gate_params = [
                (n, p) for n, p in merged_model.named_parameters() if "gate" in n
            ]
    ...
    def subspace_from_loader(
        ...
        model.eval()
        loop = tqdm(loader, desc="Extracting subspace", unit="batch", leave=False)
    ...
    def test_time_adaptation(
        ...
        task_classifier = deepcopy(classifier)
        task_classifier.clip_model.vision_model = task_model
        ...
        if self._config.get("fast_dev_run", False):
            ...
            for n, p in model.named_parameters():
                if p.requires_grad:
                    print(
                        f"Name: {n}, Shape: {p.shape}, Requires Grad: {p.requires_grad}"
                    )
            print("========================================")
        ...
        for step_idx in pbar:
            ...
            loss = _KD_loss(outputs["logits"], task_outputs["logits"], T=2.0)
        ...
        del classifier, task_classifier, tta_loader, loader
        torch.cuda.empty_cache()
    ...
    @torch.no_grad()
    def _compute_and_save_gate_stats(
        self, model: nn.Module, model_name: str, model_idx: int
    ):
        self.taskpool._test_datasets = DictConfig(
            {model_name: self._test_datasets[model_name]}
        )
